function [y] = spm_fnirs_MARA_CP(x,fs,T,L,alpha)
%_

% modified by Carolina Pletti on 15/4/2025 so that it also works if
% artifacts appear at the beginning of the signal
...
% note by Carolina Pletti: I adjusted the algorithms so that it does not
...
% (1) Artefact detecion
k = round(L/2);
[A_Idx,s2_1,s2_2] = MADetection(x,k,T,fs,alpha); %note by Carolina Pletti: this is where I modified the fuction
...
for i = 1:length(s2_2);
    if i < length(s2_2); 
        if isnan(s2_2(i)) & s2_2(i+1) > 0 %added by Carolina Pletti so that the algorithm also recognizes artifacts that appear immediately after the NaNs produced by thresholding
            q1(i) = i;
        elseif ((s2_2(i) == 0) & ((s2_2(i+1)-s2_2(i)) > 0));
           q1(i) = i;
